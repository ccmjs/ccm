...
( function () {
  ...
  const self = {
    ...
    instance: async ( component, config ) => {
      ...
      const source = self.helper.isObject( config ) && config.ccm || self.helper.isObject( component.config ) && component.config.ccm;
      if ( source ) {
    ...
    helper: {
      ...
      loadFramework: async url => {
         ...
         * @type {string[]}
         */
        const version = ( ( self.helper.isObject( url ) ? url.url : url ).match( /(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)/ ) || [ 'latest' ] )[ 0 ];

        // load needed ccm framework version if not already there
        !ccm[ version ] && await self.load( url );
