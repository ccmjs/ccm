 ...
 * @version 24.2.0
 * @changes
 * version 24.2.0 (20.12.2019):
 * - bug fix for throw exception
 * - added ccm.helper.convertComponentURL(url):obj
 * - ccm.helper.onFinish supports condition function
...
( () => {
  ...
  const self = {
    ...
    helper: {
       ...
       * @description
       * The result data contains the unique 'name', 'version' and 'index' of the component.
      ...
      convertComponentURL: url => {
        ...
        if ( !self.helper.regex( 'filename' ).test( filename ) ) return new Error( 'invalid component filename: ' + filename );
      ...
      onFinish: async ( instance, results ) => {
        ...
        // check condition
        if ( settings.condition && !( await settings.condition( instance ) ) ) return;
