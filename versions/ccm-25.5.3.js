 ...
 * @version 25.5.3
 * @changes
 * version 25.5.3 (29.05.2020)
 * - prevent unnecessary throwing of exceptions for successful loaded data
 * version 25.5.2 (29.04.2020)
 * - bug fix for ccm.load when loading big data
 * version 25.5.1 (29.04.2020)
 * - bug fix for ccm.helper.html for boolean as content
 * version 25.5.0 (21.04.2020)
 * - 'async store.set(priodata):key' allows a query for priodata.key
 * - ccm.helper.integrate sets default values also for empty strings
 * version 25.4.0 (07.04.2020)
 * - added optional callback for responsive breakpoints (config.onbreakpoint)
 * version 25.3.0 (06.04.2020)
 * - restored helper function for loading icon
 * - display loading icon on instance creation
 * - loading icon is initial content in instance elements
 ...
 * version 25.1.0 (20.03.2020)
 * - added count operation for datastores
...
( () => {
  ...
  const Datastore = function () {
    ...
    this.set = priodata => new Promise( ( resolve, reject ) => {
      ...
      if ( !self.helper.isKey( priodata.key ) && !self.helper.isObject( priodata.key ) ) reject( new Error( 'invalid dataset key: ' + priodata.key ) );
      ...
      function serverDB() {
        ...
        ( that.socket ? useWebsocket : useHttp )( prepareParams( { set: priodata } ) ).then( response => ( self.helper.isObject( priodata.key ) || response.toString() === priodata.key.toString() ? resolve : reject )( response ) ).catch( error => checkError( error, reject ) );
  ...
  const self = {
      ...
      return new Promise( ( resolve, reject ) => {
        ...
        args.forEach( ( resource, i ) => {
          ...
          function successData( data ) {
            ...
            try { if ( typeof data !== 'object' ) data = self.helper.parse( data ); } catch ( e ) {}
    ...
    instance: async ( component, config ) => {
      ...
      if ( config && config.root && config.parent ) { config.root.innerHTML = ''; config.root.appendChild( self.helper.loading( config.parent ) ); }
      ...
      function createInstance() {
        ...
        function setElement() {
          ...
          if ( instance.start ) { instance.element.innerHTML = ''; instance.element.appendChild( self.helper.loading( instance ) ); }
    ...
    helper: {
      ...
      integrate: async ( priodata, dataset, as_defaults ) => {
        ...
        for ( let key in priodata ) {
          ...
          const value = self.helper.deepValue( dataset, key );
          if ( !as_defaults || value === undefined || value === '' ) self.helper.deepValue( dataset, key, priodata[ key ] );
