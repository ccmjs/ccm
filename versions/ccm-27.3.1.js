  ...
  const Datastore = function () {
    ...
    this.set = priodata => new Promise( ( resolve, reject ) => {
      ...
      function serverDB() {
        ...
        ( that.socket ? useWebsocket : useHttp )( prepareParams( { set: priodata } ) ).then( response => ( ccm.helper.isObject( priodata.key ) || response.key.toString() === priodata.key.toString() ? resolve : reject )( response ) ).catch( error => checkError( error, reject ) );
