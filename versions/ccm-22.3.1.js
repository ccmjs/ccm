  ...
  const self = {
    ...
    instance: async ( component, config ) => {
      ...
      function createInstance() {
        ...
        if ( instance.root.id === instance.index && instance.root.parentNode && ( instance.root.parentNode.tagName === 'CCM' || instance.root.parentNode.tagName.indexOf( 'CCM-' ) === 0 ) ) watchAttributes();
    ...
    helper: {
      ...
      html: function ( html, values ) {
        ...
        // is ccm HTML Element of registered component? => evaluate
        if ( element.tagName.startsWith( 'CCM-' ) && components[ element.tagName.substr( 4 ).toLowerCase() ] ) {
          const config = self.helper.generateConfig( element );
          config.root = element;
          self.start( element.tagName.substr( 4 ).toLowerCase(), config );
        }

        // is <ccm> element with component attribute? => evaluate
        if ( element.tagName === 'CCM' && element.hasAttribute( 'component' ) ) {
          const component = element.getAttribute( 'component' );
          element.removeAttribute( 'component' );
          const config = self.helper.generateConfig( element );
          config.root = element;
          self.start( component, config );
        }
      ...
      onFinish: async ( instance, results ) => {
        ...
        // convert result data
        if ( settings.convert ) results = ( await self.load( { url: settings.convert.module, type: 'module', import: settings.convert.import } ) )( results );
