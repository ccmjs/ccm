 ...
 * @version latest (17.0.0)
 * @changes
 * version 17.0.0 (24.08.2018): improved ccm.load
 * - works with Promises (no more callback, ccm.load returns a Promise)
 * - removed cache handling (no more needed because of improved browser caching)
 * - error handling if loading of at least one resource fails (catch via Promise)
 * - ccm.load.timeout for loading resources (default is 10000 milliseconds)
 * - resources could be loaded via fetch and optional init settings
 * - support of loading ES6 modules and HTML Import
 * - dependencies 'ccm.module' and 'ccm.polymer' removed (use ccm.load instead)
 * - bug fix for initialization of default (local) ccm datastores
 * - checking og received data with self.helper.regex('json')
 * - use custom element polyfill webcomponents-lite.js v1.1.0 instead of v1.0.14
 * - ccm.helper.isInstance now checks more closely
 * - added ccm.helper.isLoadError: checks if a value is a loading error of ccm.load
 * - default method for data exchanges is POST
 * (for older version changes see ccm-16.7.0.js)
 */
...
( function () {
  ...
  const Datastore = function () {
    ...
    this.get = ( key_or_query={}, callback ) => {
      ...
      function solveDependencies( value, callback ) {
        ...
        if ( !Array.isArray( value ) && !self.helper.isObject( value) ) return callback( null );
        ...
        function recursive( arr_or_obj ) {
          ...
          for ( const i in arr_or_obj ) {
            ...
            // is an array or object? => search it recursively (not ccm dependencies, element nodes and ccm instances)
            else if ( ( Array.isArray( value ) && !self.helper.isDependency( value ) ) || ( self.helper.isObject( value ) && !self.helper.isNode( value ) && !self.helper.isInstance( value ) ) ) recursive( value );
    ...
    this.set = ( priodata, callback ) => {
      ...
      function localCache() {
        ...
        // perform callback
        callback && callback( priodata.key );
    ...
    function useHttp( params, callback ) {
      ...
      self.load( { url: my.url, params: params, method: my.method } ).then( callback ).catch( console.log );
  ...
  if ( !window.ccm ) ccm = {
    ...
    /**
     * JSONP callbacks for cross domain data exchanges via ccm.load
  ...
  const self = {
        ...
        args.forEach( ( resource, i ) => {
          ...
          function importHTML() {
            ...
            if ( 'import' in document.createElement( 'link' ) )
              ...
              } ).then( proceed ).catch( console.log );
          ...
          function loadData() {
            ...
            function fetchAPI() {
              ...
              fetch( resource.url, resource.init ).then( response => response.text() ).then( successData ).catch( error => error( error ) );
            }
    ...
    component: function ( component, config, callback ) {
      ...
      if ( typeof component === 'string' ) {
        ...
        // given string is component URL
          ...
          else self.load( component ).then( proceed ).catch( console.log );
      ...
      function proceed( component ) {
        ...
        if ( !( 'customElements' in window ) ) self.load( {
          url: 'https://cdnjs.cloudflare.com/ajax/libs/webcomponentsjs/1.1.0/webcomponents-lite.js',
          integrity: 'sha384-yEuTKRGFLhOAfHNxaZiiI23KhMelYudrPUNSUK6T5u1+deGEEKsQ89cS0sPIHjyj',
          crossorigin: 'anonymous'
        } ).then( proceed ).catch( console.log ); else return proceed();
        ...
        function proceed() {
          ...
          if ( !ccm[ version ] ) self.load( component.ccm ).then( proceed ).catch( console.log ); else proceed();
            ...
            function defineCustomElement() {
              ...
              window.customElements.define( name, class extends HTMLElement {
                ...
                static get observedAttributes() {
                  self.helper.clone( component.config );
        ...
        function proceed( index ) {
          ...
          if ( cfg && cfg.key ) {
            if ( self.helper.isObject( cfg.key ) )
              ...
              return cfg.key[ 0 ] === 'ccm.load' ? self.load( cfg.key[ 1 ] ).then( integrate ).catch( console.log ) : self.get( cfg.key[ 1 ], cfg.key[ 2 ], integrate );
            else proceed( cfg );
          }
          ...
          function proceed( cfg ) {
            ...
            */
            ...
            function setElement() {
              ...
              // watch HTML attributes of instance root element
              instance.update && watchAttributes();

              /** watches attributes of instance root element */
              function watchAttributes() {

                // instance has no update property? => abort
                if ( !instance.update ) return;

                // update property is boolean true? => use default update method
                if ( instance.update === true ) instance.update = ( key, value ) => {
                  switch ( key ) {
                    case 'ccm':
                    case 'component':
                    case 'dependency':
              ...
              function solveDependency( instance_or_array, key ) {
                ...
                switch ( action[ 0 ] ) {
                  ...
                  case 'ccm.load':
                    ...
                    action.shift();
                    setContext( action );
                    self.load.apply( null, action ).then( setResult ).catch( console.log );
                    break;
    ...
    store: ( settings, callback ) => {
      ...
      if ( typeof settings.local === 'string' || self.helper.isResourceDataObject( settings.local ) )
        self.load( settings.local ).then( proceed ).catch( console.log );
      else
    ...
    helper: {
      ...
      isInstance: function ( value ) {
        ...
        return self.helper.isObject( value ) && self.helper.isComponent( value.component ) && true;
  ...
  /**
   * timeout limit (in ms) for loading a resource
   * @type {number}
   */
