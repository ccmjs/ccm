 ...
 * @version latest (22.7.0)
 * @changes
 * version 22.7.0 (30.09.2019):
 * - added ccm.helper.hasParentContact(instance):boolean
 * - updated ccm.helper.hasDomContact(instance):boolean
 ...
 * - json2json conversion allows async functions
 * version 22.5.0 (03.09.2019):
 * - ccm.helper.generateConfig accepts many forms of HTML
 * - ccm.helper.html allows advanced settings like 'no_evaluation' (no evaluation of ccm HTML Elements)
 * - ccm HTML Elements contained in Light DOM are not evaluated
 * version 22.4.0 (28.07.2019):
 * - define of Custom Element <ccm-app>
 * - <ccm-app> instead of <ccm> in ccm.helper.html
 ...
 * version 22.2.1 (24.07.2019):
 * - bug fix for ccm.helper.html2json
...
( () => {
  ...
  const Datastore = function () {
    ...
    this.set = priodata => new Promise( ( resolve, reject ) => {
      ...
      function localCache() {
        ...
        if ( that.local[ priodata.key ] ) that.local[ priodata.key ] = self.helper.integrate( priodata, that.local[ priodata.key ] );
  ...
  const self = {
      ...
      return new Promise( ( resolve, reject ) => {
        ...
        args.forEach( ( resource, i ) => {
          ...
          function loadCSS() {
            ...
            if ( resource.attr ) element = self.helper.integrate( resource.attr, element );
    ...
    instance: async ( component, config ) => {
      ...
      function createInstance() {
        ...
        function setElement() {
          ...
          // add loading icon
          self.helper.setContent( instance.element, self.helper.loading( instance ) );
    ...
    proxy: async ( component, config ) => {
      ...
      obj.start = async cfg => await Object.assign( obj, await self.instance( component, self.helper.integrate( cfg, config ) ) ).start();
      return obj;
    },
    ...
    helper: {
       ...
       * @returns {boolean}
       */
      hasDomContact: instance => self.helper.hasParentContact( instance ) && document.contains( self.context.root( instance ).root ),
      ...
      html: function ( html, values, settings ) {
        ...
        if ( Array.isArray( html ) ) {
          ...
          for ( let i = 0; i < html.length; i++ )
            result.push( self.helper.html( html[ i ], undefined, settings ) );  // recursive call
          return result;
